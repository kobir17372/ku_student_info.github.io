...
if ($student_id === 0) {
    header("Location: error.php?error=invalid_student_id");
    exit();
}
...
// Fetch user data
$stmt = $conn->prepare("SELECT * FROM `accounting` WHERE `student id` = ?");
...
if ($loggedIn && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update database
    $stmt = $conn->prepare("UPDATE `accounting` SET `profile pic` = ?, `cover pic` = ? WHERE `student id` = ?");
...
</html>      you can add here edit functionality and how     if possible rewrite me whole code