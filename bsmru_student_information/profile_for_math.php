...
$currentProfilePic = ''; // Default profile picture
$currentCoverPic = ''; // Default cover picture

// Fetch user data
$stmt = $conn->prepare("SELECT * FROM `math` WHERE `student id` = ?");
...
if ($result->num_rows > 0) {
    ...
    if (!empty($row['profile pic'])) {
        $currentProfilePic = $row['profile pic'];
    }
    if (!empty($row['cover pic'])) {
        $currentCoverPic = $row['cover pic'];
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profile_picture']) && is_uploaded_file($_FILES['profile_picture']['tmp_name'])) {
        ...
        if (in_array($profileMimeType, $allowedTypes)) {
            $profileFilename = uniqid() . '_' . basename($profileFile['name']);
    ...
    if (isset($_FILES['cover_picture']) && is_uploaded_file($_FILES['cover_picture']['tmp_name'])) {
        ...
        if (in_array($coverMimeType, $allowedTypes)) {
            $coverFilename = uniqid() . '_' . basename($coverFile['name']);
    ...
    // Update database
    $stmt = $conn->prepare("UPDATE `math` SET `profile pic` = ?, `cover pic` = ? WHERE `student id` = ?");
